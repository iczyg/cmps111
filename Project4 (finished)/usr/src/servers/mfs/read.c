...
FORWARD _PROTOTYPE( int rw_chunk, (struct inode *rip, u64_t position,
	...
	int *completed)							);	
FORWARD _PROTOTYPE( int rw_meta, (struct inode *rip, block_t b,
	unsigned off, size_t chunk, unsigned left, int rw_flag,
	cp_grant_id_t gid, unsigned buf_off, unsigned int block_size,
	int *completed)		);
...
/*fs_metaread and fs_metawrite are based off of fs_readwrite. They are split up for user interaction purposes. */
/*===========================================================================*
 *				fs_readwrite				     *
 *===========================================================================*/
...
PUBLIC int fs_metaread(void)
{
  ...
  /* Read 'chunk' bytes. */
  r = rw_meta(rip, rip->i_zone[9], 0, nrbytes,
 ...
 *				rw_meta		    *
 *===========================================================================*/
 /*-based off of rw_chunk, had to be redone since metadata is outside normal file contents */
PRIVATE int rw_meta(rip, b, off, chunk, left, rw_flag, gid,
 buf_off, block_size, completed)
...
int *completed;			/* number of bytes copied */
{
  ...
  *completed = 0;
  ...
  if((((rip->i_mode & I_SET_STCKY_BIT) != 0) && (rip->i_zone[9] == NO_ZONE))
       ||(((rip->i_mode & I_SET_STCKY_BIT) == 0) && (rip->i_zone[9] != NO_ZONE))) {
    return(-1);
  }


  if(b == NO_BLOCK) {
    /*Allocate a if needed*/
